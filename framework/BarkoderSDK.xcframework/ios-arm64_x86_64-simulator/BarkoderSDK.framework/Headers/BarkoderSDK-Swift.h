...
/// When AR mode is <code>.matchFilter</code>, controls whether only matched results are returned.
/// If <code>false</code>, all detected results are returned, while unmatched results remain marked through <code>isMatched</code>.
/// This option only applies when <code>arMode == .matchFilter</code> and a match filter is active.
/// note:
/// Default value is <code>true</code>.
@property (nonatomic) BOOL returnOnlyMatchedResults;
/// When AR mode is <code>.matchFilter</code>, controls whether only matched results are displayed.
/// If <code>false</code>, all decoded results are displayed, including unmatched results.
/// This option only applies when <code>arMode == .matchFilter</code> and a match filter is active.
/// note:
/// Default value is <code>false</code>.
@property (nonatomic) BOOL displayOnlyMatchedResults;
...
/// AR automatically selects matched results only.
/// <code>displayOnlyMatchedResults</code> controls whether unmatched results are displayed,
/// while <code>returnOnlyMatchedResults</code> controls whether they are returned.
  BarkoderARModeMatchFilter = 4,
};
...
enum BarkoderRoiCenterMark : NSInteger;
...
/// Retrieves/Sets the style of the visual marker drawn at the center of the Region of Interest (ROI)
/// note:
/// Default value is <code>.none</code>
@property (nonatomic) enum BarkoderRoiCenterMark roiCenterMark;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
///
- (void)setCamera:(enum BarkoderCameraPosition)position SWIFT_AVAILABILITY(maccatalyst,unavailable,message="Back/front camera selection is not supported on Mac Catalyst");
/// Returns available camera devices on Mac Catalyst.
...
///
- (void)setFlash:(BOOL)enabled SWIFT_AVAILABILITY(maccatalyst,unavailable,message="Flash is not supported on Mac Catalyst");
...
- (void)isFlashAvailable:(void (^ _Nonnull)(BOOL))completion SWIFT_AVAILABILITY(maccatalyst,unavailable,message="Flash is not supported on Mac Catalyst");
...
///
- (void)configureFlashButtonWithVisible:(BOOL)visible position:(NSValue * _Nullable)position iconSize:(NSNumber * _Nullable)iconSize tintColor:(UIColor * _Nullable)tintColor backgroundColor:(UIColor * _Nullable)backgroundColor cornerRadius:(NSNumber * _Nullable)cornerRadius padding:(NSNumber * _Nullable)padding useCustomIcon:(NSNumber * _Nullable)useCustomIcon customIconFlashOn:(UIImage * _Nullable)customIconFlashOn customIconFlashOff:(UIImage * _Nullable)customIconFlashOff SWIFT_AVAILABILITY(maccatalyst,unavailable,message="Flash controls are not supported on Mac Catalyst");
...
/// Clears the current AR result cache and removes all rendered AR barcode overlays
/// without stopping the camera, ending the scanning session, or emitting results.
- (void)resetARCache;
- (void)cameraFrameReceivedWithSampleBuffer:(CMSampleBufferRef _Nonnull)sampleBuffer;
- (void)layoutSubviews;
@end
...
/// Represents the style of the visual marker displayed at the center of the Region of Interest (ROI).
typedef SWIFT_ENUM(NSInteger, BarkoderRoiCenterMark, open) {
/// No marker will be shown at the center of the ROI.
  BarkoderRoiCenterMarkNone = 0,
/// A crosshair will be drawn at the center of the ROI.
  BarkoderRoiCenterMarkCrosshair = 1,
/// A small red dot will be displayed at the center of the ROI.
  BarkoderRoiCenterMarkPoint = 2,
};
